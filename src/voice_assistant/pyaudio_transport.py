...
class PyAudioTransport(BaseTransport):
    ...
    async def start(self):
        ...
        if AUDIO_INPUT_DEVICE_INDEX is not None:
            input_params['input_device_index'] = int(AUDIO_INPUT_DEVICE_INDEX)
            print(f"✓ 使用指定的输入设备: #{AUDIO_INPUT_DEVICE_INDEX}")
        else:
        ...
        if AUDIO_OUTPUT_DEVICE_INDEX is not None:
            output_params['output_device_index'] = int(AUDIO_OUTPUT_DEVICE_INDEX)
            print(f"✓ 使用指定的输出设备: #{AUDIO_OUTPUT_DEVICE_INDEX}")
